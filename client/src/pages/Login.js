...
const Login = ({ setUser }) => {
  ...
  // Google Authentication handler using implicit grant flow
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        // Отримання інформації про профіль користувача безпосередньо через Google API
        const userInfoRes = await fetch('https://www.googleapis.com/oauth2/v3/userinfo', {
          headers: { Authorization: `Bearer ${tokenResponse.access_token}` },
        });
        const profile = await userInfoRes.json();

        // Передача профілю користувача на ваш бекенд
        ...
        alert('Не вдалося зв’язатися з сервером авторизації');
      }
    },
    onError: () => alert('Авторизацію скасовано або виникла помилка Google Auth'),
  });
            ...
            <div className="login-card-container">
              <div className="login-card">
                ...
                <form onSubmit={handleLoginSubmit}>
                  ...
                  {/* Додано роздільник та кнопку Google за макетом */}
                  ...
                  <div className="google-auth-wrapper">
                    <button type="button" className="google-custom-btn" onClick={() => handleGoogleLogin()}>
                      Google
                    </button>
                  </div>
              ...
              <div className="auth-right-panel">
                <div className="auth-form-container">
                  ...
                  <form onSubmit={handleRegisterSubmit}>
                    ...
                    {/* Додано такий самий роздільник та кнопку Google для реєстрації */}
                    <div className="or-divider-container">
                      <div className="divider-line"></div>
                      <span className="divider-text">OR CONTINUE WITH</span>
                      <div className="divider-line"></div>
                    ...
                    <div className="auth-switch" style={{ marginTop: '16px' }}>
                      Already have an account? <span onClick={() => triggerTransition(true)}>Log in</span>
                    </div>
                  </form>
                </div>
              </div>
